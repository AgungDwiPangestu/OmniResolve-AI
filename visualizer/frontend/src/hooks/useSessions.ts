...
export function useSessions(
  ...
  useEffect(() => {
    ...
    // If no new sessions, skip
    if (newSessions.length === 0) return;

    // Find a new active session (prefer same project if project root exists)
    const newSessionInProject = newSessions.find(
      (s) => s.status === "active" && (!currentProjectRoot || s.projectRoot === currentProjectRoot),
    );
