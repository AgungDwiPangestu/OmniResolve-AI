...
async def enrich_agent_with_summaries(
    ...
    name_source = (
        event_data.agent_name or event_data.agent_id or event_data.task_description or event_data.agent_type or ""
    )
    ...
    for custom_name in ["liaison", "logistics", "strategic", "supply chain", "orchestrator", "supervisor"]:
        if agent.name and custom_name in agent.name.lower():
            ...
            break
        if agent.id and custom_name in agent.id.lower():
            is_custom_agent = True
            break

    if name_source and not is_custom_agent:
