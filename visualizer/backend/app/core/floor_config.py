...
def get_cached_building_config() -> BuildingConfig:
    """Return the cached building config, falling back to floors.toml or empty."""
    global _cached_config
    if _cached_config is None:
        try:
            _cached_config = load_building_config_from_toml(toml_path=DEFAULT_TOML_PATH)
        except Exception:
...
async def load_building_config(db: AsyncSession) -> BuildingConfig:
    ...
    Falls back to floors.toml or an empty BuildingConfig if no preference is stored
    or if the stored JSON is invalid.
    ...
    # Fallback to loading from floors.toml
    try:
        config = load_building_config_from_toml(toml_path=DEFAULT_TOML_PATH)
        _cached_config = config
        return config
    except Exception as exc:
        logger.exception("Error loading fallback building config from TOML: %s", exc)
